        ...
        <div itemscope="itemscope" itemtype="http://schema.org/Comment">
                    ...
                    <div class="comment-head">
                        <div class="comment-name">
                            <span style="display:none" itemprop="datePublished">
                                <?= date('Y-m-d\TH:i:s', $model->time_create) ?>
                            </span>
                            <span class="date" itemprop="creator"><?= $model->getUserName() ?></span>
                            <span class="date"><?= Yii::$app->formatter->asDate($model->time_create, 'short') ?>
            ...
            <?php else: ?>
                <div class="comment-box">
                    <div class="comment-head">
                        <h6 class="comment-name">
                            <span style="display:none"
                                  itemprop="datePublished"><?= date('Y-m-d\TH:i:s', $model->time_create) ?></span>

                            <span itemprop="creator"><?= $model->getUserName() ?></span>

                            <span class="date"><?= Yii::$app->formatter->asDate($model->time_create, 'short') ?>
                                в <?= Yii::$app->formatter->asTime($model->time_create, 'short') ?> пишет:</span>
                        </h6>
                            ...
                            <?php if (Yii::$app->user->can(Comment::RBAC_FRONTEND_CREATE, ['status' => Yii::$app->user->status])): ?>
                                <?php if (Yii::$app->user->can(Comment::RBAC_FRONTEND_UPDATE, ['owner_id' => $model->user_id])): ?>
                                    <?php if ((time() - $model->time_create) <= Comment::COMMENT_UPDATE_DELAY): ?>
                                        <a title="Редактировать комментарий" href="/comment/create?id=<?= $model->id ?>"
                                           data-id="<?= $model->id ?>" class="modalForm"><i class="ic ic-edit"></i> </a>
                                        <a title="Удалить комментарий" href="#deleteComment"
                                           data-id="<?= $model->id ?>"><i class="ic ic-remove"></i> </a>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <a title="Жалоба на комментарий" href="#abuseComment" data-id="<?= $model->id ?>"><i
                                                class="ic ic-fire"></i></a>
                                    <a class="like" title="Оценить" href="#likeComment" data-id="<?= $model->id ?>"><i
                                                class="ic ic-thumbs-up"></i><span><?= ($model->likes ? $model->likes : "") ?></span></a>
                                <?php endif; ?>
                            <?php endif; ?>
                            <a
                                    href=""
                        ...
                        <span
                                id="onlink<?= $model->id ?>"
                                class="hidden">https://<?= $_SERVER['HTTP_HOST'] . $_SERVER["REQUEST_URI"]; ?>
                            #comment<?= $model->id ?></span>
                    ...
                    <div class="comment-content">
                        <article id="comment-text" itemprop="text">
                            <p><?= \frontend\components\OutLink::load()->process($model->text) ?></p>
                        </article>
                        <a title="Ответить на комментарий" class="pull-right reply-comment" href="#replyComment"
                           data-id="<?= $model->id ?>" data-author="<?= $model->getUserName() ?>">Ответить</a>
                    </div>
                </div>
            <?php endif; ?>
        </div>
